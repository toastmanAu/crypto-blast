...
//!   * `fixture-court.bin`            — a completing 25-turn attested match,
...
fn run_with_locks(
    ...
    // but the 25-turn match REPLAY dominates: ~179M total (measured). Under the
    // 200M per-tx ceiling — keep the fixture lean (MAX_CRATES is tuned for this).
    ctx.verify_tx(&tx, 200_000_000).map(|c| c as u64)
}
...
fn rejects_forfeit_match_already_over() {
    let (k0, k1) = signing_keys();
    // Use the FULL 25-tape fixture as the prefix → the replay sets a winner, so
    // the match is over and must settle via the court path, not forfeit.
    let setup = forfeit_claim_setup_keys(25, &k0, &k1);
    let r = run_forfeit_claim(&setup, 500, None);
    assert!(
        r.is_err(),
    ...
    // only AFTER the full 25-tape replay (check 5 follows the check-4 replay).
    match r {
...
// court fixture IS the loser-final-actor (self-destruct) shape: 25 turns, team0
// acts last on turn 24 (even index), winner=1 (see fixture-court.meta.txt).
// Player1's final-head signature covers only up to player1's last turn (turn 23)
// and does NOT cover the final move (turn 24, player0's). This is exactly
