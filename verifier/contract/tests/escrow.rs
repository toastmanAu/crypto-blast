...
//!   * `fixture-court.bin`            — a completing 39-turn attested match,
...
fn run_with_locks(
    ...
    // but the 39-turn match REPLAY dominates: ~194M total (measured). Under the
    // 200M per-tx ceiling, with little margin — keep the fixture lean.
...
fn rejects_forfeit_match_already_over() {
    let (k0, k1) = signing_keys();
    // Use the FULL 39-tape fixture as the prefix → the replay sets a winner, so
    // the match is over and must settle via the court path, not forfeit.
    let setup = forfeit_claim_setup_keys(39, &k0, &k1);
    let r = run_forfeit_claim(&setup, 500, None);
    assert!(
        r.is_err(),
    ...
    // only AFTER the full 39-tape replay (check 5 follows the check-4 replay).
    match r {
...
// court fixture IS the loser-final-actor (self-destruct) shape: 39 turns, team0
// acts last on turn 38 (even index), winner=1 (see fixture-court.meta.txt).
// Player1's final-head signature covers only up to player1's last turn (turn 37)
// and does NOT cover the final move (turn 38, player0's). This is exactly
