...
STATIC = [
    ...
    ("ape_victory",          "apeVictory",     None,             "right"),  # A7 victory / celebrate
]
...
# ICONS: weapon-wheel icons (D1). (source_stem, weaponId) — explicit map like STATIC,
# so friendly export stems (wheel_*, matching moon_rocket/gas_bomb elsewhere in raw/)
# resolve to the right weapon. Manifest key `icon<WeaponId>` carries weaponId so the
# wheel UI looks up by weapon. 512² magenta-key + alpha-trim. Written to sprites/icons/.
# Pending weapons keep a canonical icon_<weaponId> stem and skip until a master lands.
ICONS = [
    ("wheel_rocket",      "moonShot"),
    ("wheel_gas",         "gasGrenade"),
    ("wheel_crate",       "airdropCluster"),
    ("wheel_melon",       "watermelonBomb"),
    ("wheel_llama",       "llamaBomb"),
    ("wheel_portal",      "bridge"),
    ("icon_diamondHands", "diamondHands"),   # pending
    ("icon_pumpPunch",    "pumpPunch"),      # pending
    ("icon_whaleDump",    "whaleDump"),      # pending
    ("icon_honeypotMine", "honeypotMine"),   # pending
]
...
def main():
    ...
    entries += [prep_icon(*i) for i in ICONS if have(i[0])]
