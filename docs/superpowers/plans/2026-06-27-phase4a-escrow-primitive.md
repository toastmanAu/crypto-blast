...
- **`escrow-lock.args` = 136 bytes** = `player0_lockhash(32) ‖ player1_lockhash(32) ‖ nonce0_commit(32) ‖ nonce1_commit(32) ‖ deadline_block(8 LE u64)`. Pot = cell capacity.
...
- **Winner binding:** the court path reads the final `winner` (0/1/-1) and asserts the tx outputs pay the pot to the winner's lockhash (or 50/50 split on `-1`); happy path pays the mutually-agreed winner; refund splits 50/50. Output checks via `load_cell_lock_hash`/`load_cell_capacity` on `Source::Output`.
...
1. `load_script().args()` → parse 136-byte args (2 lockhashes, 2 nonce-commits, deadline).
